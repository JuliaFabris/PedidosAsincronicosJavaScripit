...
//-------------------DE REGISTRO DE PELÍCULAS------------------//    
            ...
            switch ($inputs[i].name) {
                case 'title':
                    if (!$inputs[i].value.trim()) {
                        $errors[i].innerHTML = 'Se requiere un título.';
                        $errors[i].classList.add('is-invalid');
                        validationsErrors = true;
                    } else {
                        $errors[i].innerHTML = 'Título  correcto.';
                ...
                case 'rating':
                    if (!$inputs[i].value.trim() || !Number($inputs[i].value.trim()) || $inputs[i].value.trim() < 0 || $inputs[i].value.trim() > 10) {
                        $errors[i].innerHTML = 'Indique un número entre 0 y 10.';
                        $errors[i].classList.add('is-invalid');
                        validationsErrors = true;
                    } else {
                        $errors[i].innerHTML = 'Calificación  correcta.';
                        $errors[i].classList.remove('is-invalid');
                        $errors[i].classList.add('is-valid');
                        validationsErrors = false;
                    }
                ...
                case 'release_date':
                    if (!$inputs[i].value.trim()) {
                        $errors[i].innerHTML = 'Ingrese una fecha válida.';
                        $errors[i].classList.add('is-invalid');
                        validationsErrors = true;
                    } else {
                ...
                case 'length':
                    if (!$inputs[i].value.trim() || !Number($inputs[i].value.trim()) || $inputs[i].value.trim() < 60 || $inputs[i].value.trim() > 360) {
                        $errors[i].innerHTML = 'Indique un valor entre 60 y 360.';
                        $errors[i].classList.add('is-invalid');
                        validationsErrors = true;
                    } else {
                ...
                case 'genre_id':
                    if (!$inputs[i].value.trim()) {
                        $errors[i].innerHTML = 'Indique un género.';
                        $errors[i].classList.add('is-invalid');
                        validationsErrors = true;
                    } else {
    ...
    $form.addEventListener('submit', function (event) {
        ...
        for (let i = 0; i < $errors.length; i++) {
            if ($inputs[i].value.trim() == '') {
                $errors[i].innerHTML = 'Información requerida.';
                $errors[i].classList.add('is-invalid');
                validationsErrors = true;
            }    
        }
